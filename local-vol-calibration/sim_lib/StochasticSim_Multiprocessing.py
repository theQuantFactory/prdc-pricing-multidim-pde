...
class LocalVolStochasticIRSimulation():
    def __init__(self,  domestic_shifttimes, domestic_shiftvalues, base_shifttimes, base_shiftvalues, spot_FX,
        ...
        # Initialize heston_data as None (will be set in subclasses if needed)
        if not hasattr(self, 'heston_data'):
...
#----------------------------------------------------------------------------------------------------------
        ...
        heston_data = getattr(self, 'heston_data', None)
...
#---------------------------------------------------------------------------       
    def _get_X_and_U(self):
        ...
        n_timesteps = self.tdisargs['time_steps'] + 1
        n_trials = self.n_paths_per_process
        observation_idxs = -1
        print('Reading X and U at step ', n_timesteps)
        spots = np.concatenate([np.frombuffer(xs, dtype=dtype_glob).reshape((n_trials, -1))[:,:n_timesteps][:, observation_idxs] for xs in self.Xsamples_raw], axis=0)
    ...
    def _collect_simulation_results(self):
        ...
        for obsname in observation_names:
            if (obsname.lower() == 'fxrate'):
                resout = np.concatenate([np.frombuffer(xs, dtype=dtype_glob).reshape((n_trials, -1))[:,:n_timesteps][:, observation_idxs] for xs in self.Xsamples_raw], axis=0)
...
def simulate_paths(pid, n_trials, tdisargs, sdedata, g1pp_data, heston_data, lvol_data, det_DR = False, det_FR = True, DLV=True, in_DRN=True,
    ...
    dt = T/tdisargs['time_steps'] #smp.maxT/smp.maxn_time
   
    n_timesteps = tdisargs['time_steps']+1
    ...
    bdtT = lambda t:(1.0 - np.exp(-domestic_a[0]*(T-t)))/domestic_a[0]
    #######################################################################################################

    t_stamp = np.frombuffer(smp.t_stamp_raw[pid], dtype=dtype_glob)
    t_stamp[:n_timesteps] = dt*np.arange(n_timesteps)
    ...
    xds = np.ones(n_trials)*(g1pp_data.domestic_x0 if det_DR==False else 0.0)
    xbs = np.ones(n_trials)*(g1pp_data.base_x0 if det_FR==False else 0.0)
    if (SAVE_XD):
    ...
    for idx, (t, dt) in enumerate(zip(t_stamp[1:n_timesteps], np.diff(t_stamp[:n_timesteps])), 1):
        ...
        ################# Local Volatility ################################
        dW = dW_sample[:,0, xpos]
        tidx = search_index(tslices, t) if (DLV==True) else search_index_closest(tslices, t) #search_index(tslices, t) #if (DLV==True) else np.searchsorted(tslices, t) - 1
        #print('LVOL search_index:', tidx)
        ...
        #print('pid %d, t %.3f, volvals_mean %.8f'%(pid, t, np.mean(volvals)))
        sig2o2 = volvals**2/2
        ...
        if (DLV==False):
            ...
            Ut = Ut + Udrift + Uincr
            Ut[Ut<0] = 0 #Ut = np.abs(Ut) #-1*Ut[idxmask1]
            if ((idx-1)%20==0):
                print(Ut.shape, end='')
            if (SAVE_HESTON):
                Utsamples[:,idx] = Ut
